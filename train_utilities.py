...
def parse_arguments():
    ...
    parser.add_argument("--class_0_weight", type=float, default=1, help="Weight for class 0 in Focal Loss")
    parser.add_argument("--class_1_weight", type=float, default=1, help="Weight for class 1 in Focal Loss")
...
def train(model: torch.nn.Module, train_loader: torch.utils.data.DataLoader, valid_loader: torch.utils.data.DataLoader, criterion: torch.nn.Module, optimizer: torch.optim.Optimizer, epochs: int, batch_size: int,lr: float,model_folder: str, model_name: str, num_predictions_to_log:int=5) -> None:
    ...
    for epoch in range(epochs):
        ...
        train_artifact = wandb.Artifact(f"train" + str(wandb.run.id), type="train_predictions")
        ...
        for i, data in enumerate(train_loader):
            ...
            if num_train_logged < num_predictions_to_log:
                ...
                log_predictions(input_img, label_img, pred_img, epoch, i, total_table)
                num_train_logged += 1
            plt.close("all")
