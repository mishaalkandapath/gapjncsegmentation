...
def get_neuron(imgname):
     ...
     if os.path.isfile(base+"train_neuro/"+newname):
        ...
        newname = imgname.replace("SEM_dauer_2_image_export_", "SEM_adult_gj_segmentation_WL.vsseg_export_").replace(".png.png", ".png")
        assert os.path.isfile("/Volumes/Normal/gapjnc/neuronsegmentation_export/"+newname), newname
        return "/Volumes/Normal/gapjnc/neuronsegmentation_export/"+newname
...
for img in tqdm(imgs):
    ...
    if depth == 100 or depth >108: continue
...
for i in tqdm(range(len(seq_imgs))):
    ...
    os.mkdir(BASE+"3d_train/train_gts/"+os.path.split(seq_imgs[i][1])[-1][:-4])
    os.mkdir(BASE+"3d_train/train_imgs/"+os.path.split(seq_imgs[i][1])[-1][:-4])
    os.mkdir(BASE+"3d_train/train_neuro/"+os.path.split(seq_imgs[i][1])[-1][:-4])

    for j in range(4):
        try:
            shutil.copy(seq_imgs[i][j], BASE+"3d_train/train_imgs/"+os.path.split(seq_imgs[i][1])[-1][:-4]+"/"+os.path.split(seq_imgs[i][j])[-1])
        except:
            shutil.copy(seq_imgs[i][j].replace("final_jnc_only_split/train_imgs", "image_export"), BASE+"3d_train/train_imgs/"+os.path.split(seq_imgs[i][1])[-1][:-4]+"/"+os.path.split(seq_imgs[i][j])[-1])
        ...
        seg *= 255
        cv2.imwrite(BASE+"3d_train/train_gts/"+os.path.split(seq_imgs[i][1])[-1][:-4]+"/"+os.path.split(seq_segs[i][j])[-1], seg)

        shutil.copy(seq_masks[i][j], BASE+"3d_train/train_neuro/"+os.path.split(seq_imgs[i][1])[-1][:-4]+"/"+os.path.split(seq_masks[i][j])[-1])
